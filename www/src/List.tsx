...
const List = <T extends any> ({ header, items, refresh, serialize, itemKey, select }: Props<T>) => {
  ...
  return <>
    <div className='flex gap-5'>
      ...
      <Button className="bg-inactive max-w-max px-10 h-12" text="Обновить" handler={refresh}/>
    </div>
    {
      items?.length && <>
        <div className="flex">
          ...
          <Button className="bg-dark" text="Очистить" handler={clearFilterCallback} />
        </div>
        <div className="overscroll-y-scroll flex gap-5 flex-wrap place-content-evenly">
          {
            items.filter(item => !filter.length || serialize(item).toLowerCase().includes(filter.toLowerCase())).map(item =>
              <Button className="bg-vk-magenta max-w-max px-20" key={itemKey(item)} text={serialize(item)} handler={() => select(item)}/>
            )
          }
        </div>
      </>
